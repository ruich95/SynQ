...
fir reg = 
  ...
  in mkFIR' {coefW=16} (sum2 {m=PredNTaps} 10) coefs reg 
     -- mkFIR' {coefW=16} (sum1 {m=PredNTaps}) coefs reg 

fir2: (Seq comb seq, Primitive comb, Arith comb)
   => (1 reg: Reg (Repeat PredNTaps (BitVec 16)) comb seq)
   -> seq (RepeatSt PredNTaps $ !* (BitVec 16))
          (BitVec 16) (BitVec 32)
fir2 reg = 
  let _ = repeatSig PredNTaps (BV {n=32})
  in mkFIR2 {coefW=16} coefs reg 
...
firTAC2 = 
  let _ : (St (RepeatSt PredNTaps $ !* (BitVec 16))) = repeatSt {n=PredNTaps}
  in moveState $ elimDead $ elimGet $ flatTAC $ genTAC (fir2 reg)
...
%ambiguity_depth 4
acc: (Seq comb seq, Primitive comb, Arith comb)
...
test: (Seq comb seq, Primitive comb, Arith comb)
  => seq (!* UInt8) () (UInt8)
test = Seq.pure $ proj1 (prod (add' (const $ BV {n=8} 1) (const $ BV {n=8} 1)) 
                              (add' (const $ BV {n=8} 2) (const $ BV {n=8} 3)))

test2: (Seq comb seq, Primitive comb, Arith comb)
  => seq (!* UInt8) () (UInt8)
test2 = Seq.pure $ prodElim $ proj1 (prod (add' (const $ BV {n=8} 1) (const $ BV {n=8} 1)) 
                                          (add' (const $ BV {n=8} 2) (const $ BV {n=8} 3)))                    
